...
enum class EAMsBlueprintOpenType : uint8
{
	None = 0,
	Function,
	Event
...
class FAMsObjectsViewerFilter : public IClassViewerFilter
{
...
public:
	...
	static bool OpenBlueprintEditor(UBlueprint* Blueprint, EAMsBlueprintOpenType OpenType, FName FunctionNameToOpen, bool bForceFullEditor, bool bAddBlueprintFunctionIfItDoesNotExist);
...
inline bool FAMsEditorUtilities::PickChildrenOfClass(const FText& TitleText, UClass*& OutChosenClass, UClass* Class)
{
	// Create filter
	const TSharedPtr<FAMsObjectsViewerFilter> Filter = MakeShareable(new FAMsObjectsViewerFilter);
...
inline bool FAMsEditorUtilities::OpenBlueprintEditor(UBlueprint* Blueprint, EAMsBlueprintOpenType OpenType, FName FunctionNameToOpen, bool bForceFullEditor, bool bAddBlueprintFunctionIfItDoesNotExist)
{
	if (!Blueprint)
	{
		return false;
	...
	if (OpenType != EAMsBlueprintOpenType::None && FunctionNameToOpen != NAME_None)
	{
		UClass* Class = Blueprint->GeneratedClass;
		check(Class);

		if (OpenType == EAMsBlueprintOpenType::Function)
		{
		...
		else if (OpenType == EAMsBlueprintOpenType::Event)
		{
			ObjectToFocusOn = bAddBlueprintFunctionIfItDoesNotExist
...
inline UEdGraph* FAMsEditorUtilities::BlueprintGetOrAddFunction(UBlueprint* Blueprint, FName FunctionName, UClass* FunctionClassSignature)
{
...
inline UEdGraph* FAMsEditorUtilities::BlueprintGetFunction(UBlueprint* Blueprint, FName FunctionName, UClass* FunctionClassSignature)
{
	if (!Blueprint || Blueprint->BlueprintType != BPTYPE_Normal)
	{
		return nullptr;
...
inline UK2Node_Event* FAMsEditorUtilities::BlueprintGetOrAddEvent(UBlueprint* Blueprint, FName EventName, UClass* EventClassSignature)
{
	if (!Blueprint || Blueprint->BlueprintType != BPTYPE_Normal)
	{
		return nullptr;
...
inline UK2Node_Event* FAMsEditorUtilities::BlueprintGetEvent(UBlueprint* Blueprint, FName EventName, UClass* EventClassSignature)
{
	if (!Blueprint || Blueprint->BlueprintType != BPTYPE_Normal)
	{
		return nullptr;
...
inline bool FAMsEditorUtilities::OpenEditorForAsset(const UObject* Asset)
{
